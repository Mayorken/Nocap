  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={[styles.shell, desktop && styles.desktopShell]}>
        ...
        {screen === 'home' && <Home wallet={wallet} challenges={liveChallenges} onCreate={() => go('create')} onSelect={selectChallenge} />}
...
function Home({ wallet, challenges, onCreate, onSelect }: { wallet: { address: string; balance: string } | null; challenges: Challenge[]; onCreate(): void; onSelect(item: Challenge): void }) {
  ...
  return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
    ...
    {!wallet && <View style={styles.manifestoCard}>
      ...
      {[['01','Commit','Everyone locks the same small amount.'],['02','Show up','Submit one honest receipt of the work.'],['03','Settle','Finishers reclaim their commitment on-chain.']].map(item => <View key={item[0]} style={styles.manifestoRow}><Text style={styles.manifestoStep}>{item[0]}</Text><View style={styles.manifestoCopy}><Text style={styles.manifestoTitle}>{item[1]}</Text><Text style={styles.manifestoText}>{item[2]}</Text></View></View>)}
      <Text style={styles.manifestoFoot}>Connect your wallet above to see live pacts.</Text>
    </View>}
