...
import type { Challenge, ReviewPolicy } from './src/types';
...
function AppContent() {
  ...
  const handleCreate = async () => {
    ...
    setBusy(true);
    try {
      const receipt = await createOnchainChallenge({ title: form.title.trim(), stake: form.stake, durationDays: Math.max(1, Number(form.duration)), maxParticipants: Number(form.max), reviewPolicy: form.reviewPolicy as ReviewPolicy, demo: Number(form.duration) === 0 });
...
function Create({ form, setForm, onBack, onSubmit, busy, wallet, onConnect }: { form: Record<string, string>; setForm(value: any): void; onBack(): void; onSubmit(): void; busy: boolean; wallet: unknown; onConnect(): void }) {
  ...
  return <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled"><Back onPress={onBack} /><Text style={styles.pageKicker}>NEW PACT</Text><Text style={styles.pageTitle}>Define one clear{`\n`}outcome.</Text><Text style={styles.formIntro}>Set the terms once. Your squad joins, submits proof, and settles the result on Monad.</Text>
    ...
    <Label text="PROOF REVIEW RULE" /><View style={styles.policyGrid}>{[
      { id:'host', icon:'user-check', title:'Host decides', copy:'The pact creator makes the final call.' },
      { id:'majority', icon:'users', title:'Majority vote', copy:'More than half of eligible peers must agree.' },
      { id:'unanimous', icon:'people-arrows', title:'Unanimous', copy:'Every eligible peer must approve.' },
    ].map(policy => <Pressable key={policy.id} style={[styles.policyCard, form.reviewPolicy === policy.id && styles.policyCardActive]} onPress={() => update('reviewPolicy', policy.id)}><FontAwesome5 name={policy.icon} size={15} color={form.reviewPolicy === policy.id ? colors.ink : colors.acid} /><View style={styles.policyCopy}><Text style={[styles.policyTitle, form.reviewPolicy === policy.id && styles.policyTextActive]}>{policy.title}</Text><Text style={[styles.policyText, form.reviewPolicy === policy.id && styles.policyTextActive]}>{policy.copy}</Text></View>{form.reviewPolicy === policy.id && <Ionicons name="checkmark-circle" size={18} color={colors.ink} />}</Pressable>)}</View><Text style={styles.demoHelper}>This rule is fixed on-chain when the pact is published and cannot be changed later.</Text>
