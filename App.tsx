...
function Home({ wallet, challenges, onCreate, onSelect }: { wallet: { address: string; balance: string } | null; challenges: Challenge[]; onCreate(): void; onSelect(item: Challenge): void }) {
  const display = challenges.length ? challenges : CHALLENGES;
  const featured = display[0] ?? FEATURED;
  return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
    ...
    {display.slice(1).map(item => <ChallengeCard key={item.id} item={item} onPress={() => onSelect(item)} />)}
    {!CONTRACT_ADDRESS && <Text style={styles.previewNote}>Preview mode · deploy and configure the contract to load live challenges.</Text>}
